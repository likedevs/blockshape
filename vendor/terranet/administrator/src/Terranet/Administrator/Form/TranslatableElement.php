...
use Terranet\Administrator\Form\Contracts\FormElement;
use Terranet\Administrator\Form\Contracts\Validator;
...
class TranslatableElement implements FormElement, Validator
{
	...
	public function html()
	{
		 ...
		 *
		 * @var $repository \Terranet\Translatable\Translatable;
		 */
		$repository   = $this->element->getRepository();

		/**
		 * @var $locale = \Terranet\Multilingual\Language
		 */
...
HTML;
	...
	private function localeSwitcher()
	{
		...
		{
			$buttons[] = '<button type="button" class="btn btn-default btn-sm '.($locale->id == Lang::id() ? 'active' : '').'" data-locale="'.$locale->id.'">'.$locale->slug.'</button>';
		}
