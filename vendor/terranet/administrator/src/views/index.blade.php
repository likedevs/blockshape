...
<form method="post" id="datagrid-form" action="{{ qsRoute('admin_model_global_action', ['page' => $modelName]) . $queryString }}">
    <?=Form::token()?>
    ...
    <?php
    $sortableFactory = app('scaffold.sortable');
    ?>
    <table id="example1" class="table table-bordered table-striped">
        <thead>
            <tr role="row">
                @foreach($columns as $column)
                <th style="{{ ('id' == strtolower($column->getName()) ? 'width: 50px;' : '') }}">
                    @if ($column->isSortable() && ($sortable = $column->getSortable()))
                        <a href="{{ $sortableFactory->url($sortable) }}">
                        {{ $column->getTitle() }}
                        </a>

                        <i class="pull-right fa {{ ($sortableFactory->getElement() == $sortable ? ('fa-sort-' . $sortableFactory->getDirection()) : 'fa-sort') }}"></i>
                    @else
                        {{ $column->getTitle() }}
                    @endif
                </th>
        ...
        <tbody>
            ...
            <tr>
                @foreach($columns as $column)
                <td>
                    @if(is_a($column, 'Terranet\Administrator\Columns\Column'))
    ...
    {!! $items->appends(array_except(\Input::query(), ['page']))->render() !!}
@endsection
